...
- shutdownNow()：立即**终止**线程池，并尝试**打断正在执行**的任务，并且**清空任务缓存队列**，**返回尚未执行的任务**   
...
1. 首先要清除corePoolSize 和 maximumPoolSize 
2. Worker 的作用 Worker类实现了Runnable接口，Thread t = threadFactory.newThread(w) 相当于传递进来一个Runnable任务，在线程中运行这个Runnable，Worker 的run()方法，首先执行构造器传进来的firstTask，执行完FirstTask后，在循环中不断的getTask()来取得新的任务来执行，从缓存对列中取任务，判断取到的任务是否为空，为空调用workCanExit()判断是否可以退出，线程池Stop，任务队列为空，核心线程池设置空闲存活时间并且线程数大于1，允许worker退出。     
3. 任务提交给线程池之后的处理策略
