...
1. 默认情况下，TextInputFormat对任务的切片机制是按照文件规划切片，不管文件多小，都会是一个单独的切片，都会提交给一个mapTask，这种情况下，如果有大量的小文件，就会产生大量的mapTask，处理效率及其低下
2. 优化策略：最好的办法是在数据处理系统的最前端，就将小文件合成大文件，再上传到hdfs做后续分析；如果大量小文件已经在hdfs中了，可以使用CombineFileInputFormat来进行切片，它可以将多个小文件从逻辑上规划到一个切片中，这样多个小文件就可以交给一个mapTask。
